...
class ShipRocket():
    ...
    def fetch_shiprocket_carriers(self):
        ...
        if carriers and carriers.get('courier_data'):
            for carrier_data in carriers.get('courier_data'):
                ...
                if not carrier:
                    request.env['shiprocket.carrier.service'].create(
                        {'name': carrier_data.get('name'), 'carrier_code': carrier_data.get('id')})
        else:
    ...
    def rate_request(self, carrier, recipient, shipper, order=False, picking=False, is_return=False):
        ...
        dict_response = {'price': 0.0, 'currency': False, 'error_found': False, 'courier_name': False,
                         'courier_code': False}
        ...
        try:
            ...
            if rate_request.status_code == 200:
                ...
                if rate_request.get('data'):
                    ...
                    if is_india:
                        rate_dict = {c.get('courier_name'): {'rate': c.get('rate'),
                                                             'call_before_delivery': c.get('call_before_delivery'),
                                                             'etd_hrs': c.get('etd_hours'), 'rating': c.get('rating')}
                                     for c in available_couriers}
                    else:
                        rate_dict = {c.get('courier_name'): {'rate': c.get('rate').get('rate'),
                                                             'call_before_delivery': c.get('call_before_delivery'),
                                                             'etd_hrs': c.get('etd_hours'), 'rating': c.get('rating')}
                                     for c in available_couriers}
                    ...
                    if carrier.shiprocket_courier_filter == 'default_courier' and carrier.shiprocket_courier_id:
                        ...
                        courier_name = list(rate_dict.keys())[
                            list(call_before_list).index('Available')] if rate_dict and list(rate_dict.keys())[
                            list(call_before_list).index('Available')] else ''
                    elif carrier.shiprocket_courier_filter == 'lowest_etd':
                        print("inside lowest etfd--------------------------------",rate_dict)
                        courier_name = list(rate_dict.keys())[
                            list(etd_list).index(min(etd_list))] if rate_dict and list(rate_dict.keys())[
                            list(etd_list).index(min(etd_list))] else ''
                    elif carrier.shiprocket_courier_filter == 'high_ratings':
                        print("inside high ratings---------------------------",rating_list)
                        courier_name = list(rate_dict.keys())[
                            list(rating_list).index(max(rating_list))] if rate_dict and list(rate_dict.keys())[
                            list(rating_list).index(max(rating_list))] else ''
                    else:
                        courier_name = list(rate_dict.keys())[
                            list(rate_list).index(min(rate_list))] if rate_dict and list(rate_dict.keys())[
                            list(rate_list).index(min(rate_list))] else ''
                    ...
                    carrier = request.env['shiprocket.carrier.service'].search(
                        [('name', '=', dict_response['courier_name'])], limit=1)
    ...
    def set_master_package(self, net_weight, delivery_packages):
        length = sum(pack.dimension['length'] for pack in delivery_packages)
        width = sum(pack.dimension['width'] for pack in delivery_packages)
        height = sum(pack.dimension['height'] for pack in delivery_packages)
        return {'length': length/10 or 0.5, 'width': width/10 or 0.5, 'height': height/10 or 0.5, 'weight': net_weight}
    ...
    def _get_shipping_params(self, picking, carrier):
        ...
        if picking.sale_id.partner_invoice_id.phone:
            ...
            num = number[-10:]
        delivery_packages = picking.carrier_id._get_packages_from_picking(picking,
                                                                          picking.carrier_id.shiprocket_default_package_type_id)
        # added
        net_weight = carrier._shiprocket_convert_weight(
            picking.shipping_weight if picking.shipping_weight else picking.weight)
        master_package = self.set_master_package(net_weight, delivery_packages)
        print("master_package---------------------",master_package)
        result = []
        res = {
              ...
              "length": master_package.get('length'),   # converted in cm
              "breadth": master_package.get('width'),   # converted in cm
              "height": master_package.get('height'),   # converted in cm
              "weight": master_package.get('weight'),
            }
    ...
    def send_shipping(self, picking, carrier, is_return=False):
        ...
        """
        dict_response = {'order_id': 0, 'shipment_id': 0, 'status': False, 'tracking_number': 0, 'label_url': '',
                         'invoice_url': '', 'exact_price': '', 'courier_name': ''}
        param = self._get_shipping_params(picking, carrier)
        print("param-----------------------",param)
        try:
            rate_request = self.rate_request(carrier, picking.partner_id,
                                             picking.picking_type_id.warehouse_id.partner_id, order=picking.sale_id,
            ...
            if rate_request.get('error_message'):
                ...
                dict_response.update(
                    {'exact_price': rate_request.get('price'), 'courier_name': rate_request.get('courier_name'),
                     'courier_code': rate_request.get('courier_code') if rate_request.get('courier_code') else False})
            res = self._make_api_request('external/orders/create/adhoc', 'post', param[0])
            if res and res.status_code == 200:
                result = res.json()
                print("result-------------create--------",result)
                dict_response.update({'order_id': result.get('order_id') if result.get('order_id') else False,
                                      'shipment_id': result.get('shipment_id') if result.get('shipment_id') else False,
                                      'status': result.get('status') if result.get('status') else False})
                if result.get('shipment_id'):
                    ...
                    dict_response['tracking_number'] = aws_data.get('response').get('data').get(
                        'awb_code') if aws_data and aws_data.get('response') and aws_data.get('response').get(
                        'data').get('awb_code') else False
                    ...
                    if carrier.shiprocket_invoice_generate:
                        ...
                        dict_response['invoice_url'] = invoice_result.get(
                            'invoice_url') if invoice_result and invoice_result.get('invoice_url') else False
                    if carrier.shiprocket_label_generate:
                        ...
                        dict_response['label_url'] = label_result.get('label_url') if label_result and label_result.get(
                            'label_url') else False
